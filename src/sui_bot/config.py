...
class Settings:
    ...
    # --- فروشگاه کارت‌به‌کارت (v2) ---
    store_enabled: bool
    store_data_dir: str
    # --- درگاه زرین‌پال + کد تخفیف + اکانت تست (v3) ---
    zarinpal_merchant_id: str
    zarinpal_sandbox: bool
    zarinpal_min_toman: int
    zarinpal_callback_base: str
    zarinpal_callback_port: int
    ...
    def from_env(cls) -> "Settings":
        settings = cls(
            ...
            max_requests_per_window=env("MAX_REQUESTS_PER_WINDOW", 25, cast=int),
            rate_limit_seconds=env("RATE_LIMIT_SECONDS", 1, cast=int),
            block_duration=env("BLOCK_DURATION", 120, cast=int),
            ...
            store_enabled=env_bool("STORE_ENABLED", False),
            store_data_dir=str(env("STORE_DATA_DIR", "store_data")),
            zarinpal_merchant_id=str(env("ZARINPAL_MERCHANT_ID", "")).strip(),
            zarinpal_sandbox=env_bool("ZARINPAL_SANDBOX", False),
            zarinpal_min_toman=env("ZARINPAL_MIN_TOMAN", 50_000, cast=int),
            zarinpal_callback_base=str(env("ZARINPAL_CALLBACK_BASE", "")).strip().rstrip("/"),
            zarinpal_callback_port=env("ZARINPAL_CALLBACK_PORT", 8098, cast=int),
            ...
            ) or (env("ADMIN_TELEGRAM_ID", required=True, cast=int),),
        )
        settings.validate()
        return settings
    ...
    def validate(self) -> None:
        ...
        if self.zarinpal_min_toman <= 0:
            raise RuntimeError("ZARINPAL_MIN_TOMAN must be a positive integer")
        if not 1 <= self.zarinpal_callback_port <= 65535:
            raise RuntimeError("ZARINPAL_CALLBACK_PORT must be between 1 and 65535")
        try:
